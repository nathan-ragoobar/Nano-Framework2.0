...
class GPT(nn.Module):
    ...
    def from_checkpoint(cls, checkpoint_path):
        """Loads model weights from a PyTorch .pt checkpoint file"""
        print(f"Loading model from checkpoint: {checkpoint_path}")
        
        # Load the saved checkpoint
        checkpoint = torch.load(checkpoint_path, map_location='cpu', weights_only=False)
        
        # Get configuration from the checkpoint or use default
        if 'config' in checkpoint:
            config_dict = checkpoint['config']
            # Handle both dictionary and config object cases
            if hasattr(config_dict, 'n_layer'):
                # Extract attributes from config object
                config = GPTConfig(
                    block_size=getattr(config_dict, 'block_size', 1024),
                    vocab_size=getattr(config_dict, 'vocab_size', 50257),
            ...
            # Try to infer config from the model state dict
            state_dict = checkpoint['model_state_dict'] if 'model_state_dict' in checkpoint else checkpoint
            sample_key = [k for k in state_dict.keys() if 'transformer.h' in k][0]
            layer_count = max([int(k.split('.')[2]) for k in state_dict.keys() if 'transformer.h' in k]) + 1
            
            # Determine embedding dimension from weight shapes
            embd_key = 'transformer.wte.weight'
            n_embd = state_dict[embd_key].shape[1] if embd_key in state_dict else 768
            
            # Try to determine number of heads, fallback to reasonable defaults
            if layer_count <= 12:
                n_head = 12
            elif layer_count <= 24:
                n_head = 16
            ...
            print(f"Inferred config: layers={layer_count}, heads={n_head}, embd={n_embd}")
        
        # Create a new model with the right configuration
        model = cls(config)
        ...
        # Check compatibility and load weights
        ...
        for k, v in state_dict.items():
            if k in model_state_dict and model_state_dict[k].shape == v.shape:
                ...
                print(f"Skipping parameter {k} with shape mismatch or not found in model")
...
def pad_vocab(tensor, multiple=128, value=0):
    ...
    V, C = tensor.shape
    print(f"Vocabulary size in loaded model: {V}")
...
if __name__ == "__main__":
    ...
    if args.checkpoint:
        # Load from .pt file
        model = GPT.from_checkpoint(args.checkpoint)
    elif args.model[0] == "d":
        ...
        # load the GPT-2 model weights from HuggingFace
    ...
    if master_process and args.write_tensors and (not args.inference_only):
        ...
        write_model(model, f"test_gpt2_{model_size_str}.bin", dtype="float32")
        write_model(model, f"test_gpt2_{model_size_str}_bf16.bin", dtype="bfloat16")
