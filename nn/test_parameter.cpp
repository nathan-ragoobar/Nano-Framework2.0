...
TEST(ConstantFillTest, FixedPointFill) {
    // Setup
    std::vector<fixed_point_15pt16> data(10);
    absl::Span<fixed_point_15pt16> span(data); //Provides a safe view into contiguous data without owning it
    
    // Test fill with 1.5
    ConstantFill(span, fixed_point_15pt16(1.5f));
...
TEST(UniformFillTest, FixedPointRangeTest) {
    // Setup
    std::vector<fixed_point_15pt16> data(1000);
    absl::Span<fixed_point_15pt16> span(data);
    fixed_point_15pt16 min_val(-1.0f);
    fixed_point_15pt16 max_val(1.0f);
    ...
    // Verify randomness
    fixed_point_15pt16 first_val = span[0];
    bool all_same = true;
    for(size_t i = 1; i < span.size(); i++) {
        if(span[i] != first_val) {
            all_same = false;
...
TEST(NormalFillTest, FixedPointDistribution) {
    ...
    fixed_point_15pt16 mean(0.0f);
    fixed_point_15pt16 std(1.0f);
...
TEST(KaimingUniformFillTest, FixedPointTest) {
    std::vector<fixed_point_15pt16> data(100);
    absl::Span<fixed_point_15pt16> span(data);
    
    nn::ManualSeed(42);
    nn::KaimingUniformFill(span, 10);
    
    fixed_point_15pt16 expected_bound = sqrt(fixed_point_15pt16(0.2f)); //Kaiming fill follows a Gaussian distribution with std dev = sqrt(2 / in_features) 
...
TEST(UpperTriangularTest, FixedPointTest) {
    ...
    std::vector<fixed_point_15pt16> data(size * size);
    
    // Create tensor map from data
    TTypes<fixed_point_15pt16, 2>::Tensor matrix(data.data(), size, size);
    
    // Initialize to zero
    for (int i = 0; i < size * size; i++) {
        data[i] = fixed_point_15pt16(0.0f);
    }
...
TEST(OneHotTest, FixedPointTest) {
    ...
    std::vector<fixed_point_15pt16> label_data(batch_size * num_classes, fixed_point_15pt16(0.0f));
    
    TTypes<int>::ConstFlat target(target_data.data(), batch_size);
    TTypes<fixed_point_15pt16>::Matrix label(label_data.data(), batch_size, num_classes);
...
TEST(DataTypeTest, FixedPointDataType) {
    // Runtime checks
    EXPECT_EQ(DataTypeToEnum<fixed_point_15pt16>::v(), DT_FIXED);
    ...
    static_assert(std::is_same<
        ...
        fixed_point_15pt16
    >::value, "DT_FIXED should map to fixed_point_15pt16");
    
    static_assert(IsValidDataType<fixed_point_15pt16>::value,
                 "fixed_point_15pt16 should be a valid type");
    
    static_assert(DataTypeToEnum<fixed_point_15pt16>::value == DT_FIXED,
                 "Static value should match enum");
}


TEST(TypeDefinitionTest, fixed_point_15pt16TypeExists) {
    // Compile-time check that type exists
    static_assert(sizeof(fixed_point_15pt16) > 0, 
                 "fixed_point_15pt16 type not defined");
    
    // Runtime check type has expected properties
    fixed_point_15pt16 test_val(1.0f);
...
TEST_F(ParameterFixedPointTest, LazyAllocationInitializesToZero) {
    Parameter p(DT_FIXED);
    p.LazyAllocate(3);
    auto data = p.span<fixed_point_15pt16>();
...
TEST_F(ParameterFixedPointTest, GradientLazyAllocation) {
    ...
    auto grad = param->span_grad<fixed_point_15pt16>();
...
TEST_F(ParameterFixedPointTest, ZeroDataOperation) {
    auto data = param->span<fixed_point_15pt16>();
    data[0] = fixed_point_15pt16(1.5f);
...
TEST_F(ParameterFixedPointTest, ZeroGradOperation) {
    param->LazyAllocateGradient();
    auto grad = param->span_grad<fixed_point_15pt16>();
    grad[0] = fixed_point_15pt16(1.5f);
...
TEST_F(ParameterFixedPointTest, RawDataAccess) {
    auto* data_ptr = param->data<fixed_point_15pt16>();
    EXPECT_NE(data_ptr, nullptr);
    
    data_ptr[0] = fixed_point_15pt16(2.5f);
    EXPECT_EQ(data_ptr[0].to_float(), 2.5f);
}

TEST_F(ParameterFixedPointTest, RawGradAccess) {
    param->LazyAllocateGradient();
    auto* grad_ptr = param->grad<fixed_point_15pt16>();
    EXPECT_NE(grad_ptr, nullptr);
    
    grad_ptr[0] = fixed_point_15pt16(3.5f);
    EXPECT_EQ(grad_ptr[0].to_float(), 3.5f);
}

// Span Access Tests
TEST_F(ParameterFixedPointTest, SpanAccess) {
    auto span = param->span<fixed_point_15pt16>();
    EXPECT_EQ(span.size(), 5);
    
    span[0] = fixed_point_15pt16(4.5f);
    EXPECT_EQ(span[0].to_float(), 4.5f);
}

TEST_F(ParameterFixedPointTest, GradSpanAccess) {
    param->LazyAllocateGradient();
    auto grad_span = param->span_grad<fixed_point_15pt16>();
    EXPECT_EQ(grad_span.size(), 5);
    
    grad_span[0] = fixed_point_15pt16(5.5f);
...
TEST_F(TensorTest, Tensor4DAccess) {
    ...
    tensor4d(0, 0, 0, 0) = fixed_point_15pt16(4.5f);
    EXPECT_EQ(tensor4d(0, 0, 0, 0).to_float(), 4.5f);
}

// Gradient Tests
TEST_F(TensorTest, GradientTensorAccess) {
    auto flat_grad = param->flat_grad<fixed_point_15pt16>();
    auto matrix_grad = param->matrix_grad<fixed_point_15pt16>(6, 4);
    auto tensor3d_grad = param->tensor_3d_grad<fixed_point_15pt16>(2, 3, 4);
    auto tensor4d_grad = param->tensor_4d_grad<fixed_point_15pt16>(2, 2, 2, 3);
    
    flat_grad(0) = fixed_point_15pt16(5.5f);
    EXPECT_EQ(flat_grad(0).to_float(), 5.5f);
    
    matrix_grad(0, 0) = fixed_point_15pt16(6.5f);
...
TEST(ResidualTest, FixedPointOperations) {
    ...
    auto x_span = x.span<fixed_point_15pt16>();
    auto Fx_span = Fx.span<fixed_point_15pt16>();
    
    x_span[0] = fixed_point_15pt16(1.0f);
    Fx_span[0] = fixed_point_15pt16(2.0f);
    
   Residual::Forward(x.const_flat<fixed_point_15pt16>(),    // Changed to const_flat
                     Fx.const_flat<fixed_point_15pt16>(),     // Changed to const_flat
                     Hx.flat<fixed_point_15pt16>());
                     
    EXPECT_EQ(Hx.flat<fixed_point_15pt16>()(0).to_float(), 3.0f);
}
...
TEST(ResidualTest, BackwardFixedPoint) {
    ...
    auto Hx_grad_span = Hx_grad.span<fixed_point_15pt16>();
    Hx_grad_span[0] = fixed_point_15pt16(1.0f);
    Hx_grad_span[1] = fixed_point_15pt16(2.0f);
    Hx_grad_span[2] = fixed_point_15pt16(3.0f);

    // Call backward
    Residual::Backward(Hx_grad.const_flat<fixed_point_15pt16>(),
                      x_grad.flat<fixed_point_15pt16>(),
                      Fx_grad.flat<fixed_point_15pt16>());

    // Verify gradients accumulated correctly
    auto x_grad_span = x_grad.span<fixed_point_15pt16>();
    auto Fx_grad_span = Fx_grad.span<fixed_point_15pt16>();
...
TEST(NewGELUTest, ForwardFixedPoint) {
    ...
    auto x_span = x.span<fixed_point_15pt16>();
    x_span[0] = fixed_point_15pt16(1.0f);
    x_span[1] = fixed_point_15pt16(0.0f);
    x_span[2] = fixed_point_15pt16(-1.0f);
    
    NewGELU::Forward(x.const_flat<fixed_point_15pt16>(),
                    y.flat<fixed_point_15pt16>());
    
    auto y_span = y.span<fixed_point_15pt16>();
...
TEST(NewGELUTest, BackwardFixedPoint) {
    ...
    auto x_span = x.span<fixed_point_15pt16>();
    auto y_grad_span = y_grad.span<fixed_point_15pt16>();
    
    x_span[0] = fixed_point_15pt16(1.0f);
    x_span[1] = fixed_point_15pt16(-1.0f);
    y_grad_span[0] = fixed_point_15pt16(1.0f);
    y_grad_span[1] = fixed_point_15pt16(1.0f);
    
    NewGELU::Backward(x.const_flat<fixed_point_15pt16>(),
                     y_grad.const_flat<fixed_point_15pt16>(),
                     x_grad.flat<fixed_point_15pt16>());

    //std::cout << x.span<fixed_point_15pt16>()[1].to_float() << std::endl;
    //std::cout << y_grad.span<fixed_point_15pt16>()[1].to_float() << std::endl;
    //std::cout << x_grad.span<fixed_point_15pt16>()[1].to_float() << std::endl;
