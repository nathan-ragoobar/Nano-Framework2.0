...
TEST(ConstantFillTest, FixedPointFill) {
    // Setup
    std::vector<Type> data(10);
    absl::Span<Type> span(data); //Provides a safe view into contiguous data without owning it
    
    // Test fill with 1.5
    ConstantFill(span, Type(1.5f));
    
    // Verify
    for(const auto& val : span) {
        EXPECT_EQ(float(val), 1.5f);
    }
}
...
TEST(UniformFillTest, FixedPointRangeTest) {
    // Setup
    std::vector<Type> data(1000);
    absl::Span<Type> span(data);
    Type min_val(-1.0f);
    Type max_val(1.0f);
...
TEST(NormalFillTest, FixedPointDistribution) {
    ...
    for(const auto& val : span) {
        float diff = float(val) - empirical_mean;
...
TEST(KaimingUniformFillTest, FixedPointTest) {
    ...
    Type expected_bound = sqrt(Type(0.2f)); //Kaiming fill follows a Gaussian distribution with std dev = sqrt(2 / in_features) 
...
TEST(UpperTriangularTest, FixedPointTest) {
    ...
    std::vector<Type> data(size * size);
    
    // Create tensor map from data
    TTypes<Type, 2>::Tensor matrix(data.data(), size, size);
    ...
    for(int i = 0; i < size; i++) {
        for(int j = 0; j <= i; j++) {
            EXPECT_EQ(float(matrix(i,j)), 0.0f);
        }
    }
    ...
    for(int i = 0; i < size; i++) {
        for(int j = i + 1; j < size; j++) {
            EXPECT_EQ(float(matrix(i,j)), kMinValue);
        }
    }
}
...
TEST(OneHotTest, FixedPointTest) {
    ...
    std::vector<Type> label_data(batch_size * num_classes, Type(0.0f));
    
    TTypes<int>::ConstFlat target(target_data.data(), batch_size);
    TTypes<Type>::Matrix label(label_data.data(), batch_size, num_classes);
    ...
    for(int i = 0; i < batch_size; i++) {
        for(int j = 0; j < num_classes; j++) {
            if(j == target_data[i]) {
                EXPECT_EQ(float(label(i,j)), 1.0f);
            } else {
                EXPECT_EQ(float(label(i,j)), 0.0f);
            }
        }
    }
}
...
TEST(DataTypeTest, FixedPointDataType) {
    // Runtime checks
    EXPECT_EQ(DataTypeToEnum<Type>::v(), DT_FIXED);
    ...
    static_assert(std::is_same<
        ...
        Type
    >::value, "DT_FIXED should map to Type");
    
    static_assert(IsValidDataType<Type>::value,
                 "Type should be a valid type");
    
    static_assert(DataTypeToEnum<Type>::value == DT_FIXED,
                 "Static value should match enum");
}


TEST(TypeDefinitionTest, TypeTypeExists) {
...
TEST_F(ParameterFixedPointTest, ZeroGradOperation) {
    ...
    param->ZeroGrad();
    EXPECT_EQ(float(grad[0]), 0.0f);
}

// Data Access Tests
TEST_F(ParameterFixedPointTest, RawDataAccess) {
    auto* data_ptr = param->data<Type>();
    EXPECT_NE(data_ptr, nullptr);
    
    data_ptr[0] = Type(2.5f);
    EXPECT_EQ(float(data_ptr[0]), 2.5f);
}

TEST_F(ParameterFixedPointTest, RawGradAccess) {
    param->LazyAllocateGradient();
    auto* grad_ptr = param->grad<Type>();
    EXPECT_NE(grad_ptr, nullptr);
    
    grad_ptr[0] = Type(3.5f);
    EXPECT_EQ(float(grad_ptr[0]), 3.5f);
}

// Span Access Tests
TEST_F(ParameterFixedPointTest, SpanAccess) {
    auto span = param->span<Type>();
    EXPECT_EQ(span.size(), 5);
    
    span[0] = Type(4.5f);
    EXPECT_EQ(float(span[0]), 4.5f);
}

TEST_F(ParameterFixedPointTest, GradSpanAccess) {
    param->LazyAllocateGradient();
    auto grad_span = param->span_grad<Type>();
    EXPECT_EQ(grad_span.size(), 5);
    
    grad_span[0] = Type(5.5f);
    EXPECT_EQ(float(grad_span[0]), 5.5f);
}
...
TEST_F(TensorTest, Tensor4DAccess) {
    ...
    tensor4d(0, 0, 0, 0) = Type(4.5f);
    EXPECT_EQ(float(tensor4d(0, 0, 0, 0)), 4.5f);
}

// Gradient Tests
TEST_F(TensorTest, GradientTensorAccess) {
    auto flat_grad = param->flat_grad<Type>();
    auto matrix_grad = param->matrix_grad<Type>(6, 4);
    auto tensor3d_grad = param->tensor_3d_grad<Type>(2, 3, 4);
    auto tensor4d_grad = param->tensor_4d_grad<Type>(2, 2, 2, 3);
    
    flat_grad(0) = Type(5.5f);
    EXPECT_EQ(float(flat_grad(0)), 5.5f);
    
    matrix_grad(0, 0) = Type(6.5f);
    EXPECT_EQ(float(matrix_grad(0, 0)), 6.5f);
}
...
TEST(ResidualTest, FixedPointOperations) {
    ...
    auto x_span = x.span<Type>();
    auto Fx_span = Fx.span<Type>();
    
    x_span[0] = Type(1.0f);
    Fx_span[0] = Type(2.0f);
    
   Residual::Forward(x.const_flat<Type>(),    // Changed to const_flat
                     Fx.const_flat<Type>(),     // Changed to const_flat
                     Hx.flat<Type>());
                     
    EXPECT_EQ(float(Hx.flat<Type>()(0)), 3.0f);
}
...
TEST(ResidualTest, BackwardFixedPoint) {
    ...
    auto Hx_grad_span = Hx_grad.span<Type>();
    Hx_grad_span[0] = Type(1.0f);
    Hx_grad_span[1] = Type(2.0f);
    Hx_grad_span[2] = Type(3.0f);

    // Call backward
    Residual::Backward(Hx_grad.const_flat<Type>(),
                      x_grad.flat<Type>(),
                      Fx_grad.flat<Type>());

    // Verify gradients accumulated correctly
    auto x_grad_span = x_grad.span<Type>();
    auto Fx_grad_span = Fx_grad.span<Type>();

    for(int i = 0; i < 3; i++) {
        EXPECT_EQ(float(x_grad_span[i]), float(Hx_grad_span[i]));
        EXPECT_EQ(float(Fx_grad_span[i]), float(Hx_grad_span[i]));
    }
}
...
TEST(NewGELUTest, ForwardFixedPoint) {
    ...
    NewGELU::Forward(x.const_flat<Type>(),
                    y.flat<Type>());
    
    auto y_span = y.span<Type>();
    EXPECT_NEAR(float(y_span[0]), 0.841f, 0.01f);
    EXPECT_NEAR(float(y_span[1]), 0.0f, 0.01f);
    EXPECT_NEAR(float(y_span[2]), -0.159f, 0.01f);
}
...
TEST(NewGELUTest, BackwardFixedPoint) {
    ...
    auto x_span = x.span<Type>();
    auto y_grad_span = y_grad.span<Type>();
    
    x_span[0] = Type(1.0f);
    x_span[1] = Type(-1.0f);
    y_grad_span[0] = Type(1.0f);
    y_grad_span[1] = Type(1.0f);
    
    NewGELU::Backward(x.const_flat<Type>(),
                     y_grad.const_flat<Type>(),
                     x_grad.flat<Type>());

    //std::cout << x.span<Type>()[1].to_float() << std::endl;
    //std::cout << y_grad.span<Type>()[1].to_float() << std::endl;
    //std::cout << x_grad.span<Type>()[1].to_float() << std::endl;
    
    auto x_grad_span = x_grad.span<Type>();
    EXPECT_NEAR(float(x_grad_span[0]), 1.083f, 0.01f);
    EXPECT_NEAR(float(x_grad_span[1]), 0.084f, 0.01f);
}
/**/
