...
struct GPT {
  ...
  GPT(int block_size, int vocab_size, int padded_vocab_size, int n_layer,
    ...
    for (int i = 0; i < n_layer; ++i) {
      h_.emplace_back(std::make_unique<FFFBlock>(block_size, n_head, n_embed));
    }
...
*/
 ...
 public:
  ...
  std::vector<std::unique_ptr<FFFBlock>> h_;
...
struct FastFeedforwardGPT {
  using Type = floatX;

  FastFeedforwardGPT(int block_size, int vocab_size, int padded_vocab_size, int n_layer,
      int n_head, int n_embed)
      : block_size_(block_size),
        vocab_size_(vocab_size),
        padded_vocab_size_(padded_vocab_size),
  ...
  void __Forward(typename TTypes<int>::ConstMatrix idx) {
    PROFILE_TRACE_FN("FastFeedforwardGPT");
 ...
 public:
  ...
  std::unique_ptr<nn::Embedding> wte_;
  std::unique_ptr<nn::Embedding> wpe_;
  std::vector<std::unique_ptr<FFFBlock>> h_;  // Changed from Block to FFFBlock
  std::unique_ptr<nn::LayerNorm> lnf_;
  std::unique_ptr<nn::SoftmaxCrossEntropy> softmax_cross_entropy_;
...
// Original GPT class remains for backward compatibility
struct GPT {
