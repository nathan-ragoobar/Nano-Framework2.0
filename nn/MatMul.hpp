...
struct MatMul {
  ...
  static void Forward(typename TTypes<FixedT>::ConstMatrix x1,
                      typename TTypes<FixedT>::ConstMatrix x2,
                      typename TTypes<FixedT>::Matrix y, FixedT scale = FixedT(1.0f)) {
    // x: [M, N], x2: [N, K], y: [M, K]
    ...
    y.device(g_device) = (x1.contract(x2, product_dims) * scale).eval();
  }

  static void Backward(typename TTypes<FixedT>::ConstMatrix x1,
                       typename TTypes<FixedT>::ConstMatrix x2,
                       typename TTypes<FixedT>::ConstMatrix y_grad,
                       typename TTypes<FixedT>::Matrix x1_grad,
                       typename TTypes<FixedT>::Matrix x2_grad, FixedT scale = FixedT(1.0f)) {
    // input:
    // x1: [M, N], x2:[N, K]
    // y_grad: [M, K]
    //
    ...
    x1_grad.device(g_device) += (y_grad.contract(x2, product_dims) * scale).eval();
    ...
    x2_grad.device(g_device) += (x1.contract(y_grad, product_dims2) * scale).eval();
  }
  ...
  static void Forward(typename TTypes<T>::ConstMatrix x1,
    ...
    static void Backward(typename TTypes<T>::ConstMatrix x1,
        ...
        typename TTypes<T>::Matrix x2_grad, T scale = T(1.0f)) {
    // input:
    // x1: [M, N], x2:[N, K]
    // y_grad: [M, K]
    //
