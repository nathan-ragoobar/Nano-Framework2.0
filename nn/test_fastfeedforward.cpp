...
TEST_F(FastFeedforwardTest, SimpleTrainingTest) {
    ...
    for (int epoch = 0; epoch < epochs; epoch++) {
        ...
        for (auto param : params) {
            if (param->grad<float>() != nullptr) {  // Check if gradient exists using has_gradient()
                auto weights = param->span<float>();
                auto grads = param->span_grad<float>();  // Use span_grad instead of gradient_span
                for (size_t i = 0; i < param->size(); i++) {
                    weights[i] -= learning_rate * grads[i];
                }
                // Zero gradients for next iteration
                param->ZeroGrad();  // Use ZeroGrad instead of ZeroGradient
            }
        }
