...
//
  ...
  #include "util_nothing.hpp"
  #include "util_utype_helper.hpp"
  ...
  /*
    ...
    fixed_point() : data(0) {}
    fixed_point(const          char  n) : data(clamp_value(signed_value_type(n)))  { }
    fixed_point(const   signed char  n) : data(clamp_value(signed_value_type(n)))  { }
    fixed_point(const unsigned char  n) : data(clamp_value(signed_value_type(n)))  { }
    fixed_point(const   signed short n) : data(clamp_value(signed_value_type(n)))  { }
    fixed_point(const unsigned short n) : data(clamp_value(signed_value_type(n)))  { }
    fixed_point(const   signed int   n) : data(clamp_value(signed_value_type(n)))  { }
    fixed_point(const unsigned int   n) : data(clamp_value(signed_value_type(n)))  { }
    fixed_point(const   signed long  n) : data(clamp_value(signed_value_type(n)))  { }
    fixed_point(const unsigned long  n) : data(clamp_value(signed_value_type(n)))  { }
    fixed_point(const float&  f)        : data(clamp_float(f)) { }
    fixed_point(const double& d)        : data(clamp_float(d)) { }
    fixed_point(const long double& l)   : data(clamp_float(l)) { }
*/
    ...
    fixed_point& calculate_sqrt() {
        ...
        unsigned_value_type x = data;
        unsigned_value_type result = 0;
        unsigned_value_type bit = unsigned_value_type(1) << (std::numeric_limits<unsigned_value_type>::digits - 2);
        ...
        // Adjust the result for the fixed-point representation.
        data = result << (decimal_split / 2);
        return *this;
    }
    /*
    fixed_point& calculate_sqrt()
  ...
  */
  ...
  private:
  // Add clamp_value and clamp_float functions
  signed_value_type clamp_value(signed_value_type val) {
    if(val > value_max().data) return value_max().data;
    if(val < value_min().data) return value_min().data;
    return val << decimal_split;
}

  template<typename FloatType>
  signed_value_type clamp_float(FloatType f) {
    FloatType scaled = f * decimal_split_value;
    if(scaled > value_max().data) return value_max().data;
    if(scaled < value_min().data) return value_min().data;
    return signed_value_type(scaled);
}
