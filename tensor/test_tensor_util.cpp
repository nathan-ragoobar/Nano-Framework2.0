...
TEST_F(TensorUtilTest, FlatTensorCreation) {
    ...
    for (int i = 0; i < length; ++i) {
        EXPECT_FLOAT_EQ(flat(i).toFloat(), floatX(i).toFloat());
    }
    ...
    for (int i = 0; i < length; ++i) {
        EXPECT_FLOAT_EQ(const_flat1(i).toFloat(), floatX(i).toFloat());
    }
    ...
    for (int i = 0; i < length; ++i) {
        EXPECT_FLOAT_EQ(const_flat2(i).toFloat(), floatX(i).toFloat());
    }
}
...
TEST_F(TensorUtilTest, MatrixCreation) {
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            EXPECT_FLOAT_EQ(matrix(i, j).toFloat(), 
                           floatX(i * cols + j).toFloat());
        }
    }
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            EXPECT_FLOAT_EQ(const_matrix1(i, j).toFloat(), 
                           floatX(i * cols + j).toFloat());
        }
    }
    ...
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            EXPECT_FLOAT_EQ(const_matrix2(i, j).toFloat(), 
                           floatX(i * cols + j).toFloat());
        }
    }
}
    ...
    for (int i = 0; i < dim0; ++i) {
        for (int j = 0; j < dim1; ++j) {
            for (int k = 0; k < dim2; ++k) {
                ...
                EXPECT_FLOAT_EQ(tensor3d(i, j, k).toFloat(), 
                               floatX(index).toFloat());
            }
        }
    }
            ...
            for (int k = 0; k < dim2; ++k) {
                for (int l = 0; l < dim3; ++l) {
                    int index = i * (dim1 * dim2 * dim3) + 
                               j * (dim2 * dim3) + 
                               k * dim3 + l;
                    EXPECT_FLOAT_EQ(tensor4d(i, j, k, l).toFloat(), 
                                   floatX(index).toFloat());
                }
            }
        }
